...
1. **Read the raw content.** If the user provided a file path, read it from the workspace filesystem. If they pasted text, use the text directly.
2. **Read the audience profile.** Read `reference/audience-profile.md` for the default audience profile. Extract key details: role, pain points, goals, language style. If the user specifies a different audience, use their description instead.
...
1. Save the file to `output/talking-points.md`
2. Give a brief summary: how many points extracted, which categories had the strongest material, and any notable gaps (e.g., "No Data Nuggets — the source didn't contain quantitative claims")
